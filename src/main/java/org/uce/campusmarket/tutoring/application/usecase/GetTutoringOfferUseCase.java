...
public class GetTutoringOfferUseCase {

    private final TutoringOfferRepository repository;
    private final UserRepository userRepository;
    private final ReviewRepository reviewRepository;

    public TutoringOfferResponse execute(UUID id) {

        TutoringOffer offer = repository.findById(id)
                .orElseThrow(() ->
                        new DomainException("La oferta de tutoría no existe")
                );
