...
function createCard(listing) {
    ...
    const imageUrl = listing.images?.length ? listing.images[0].url : 'https://placehold.co/400x300?text=Sin+Imagen';

    const rep = Math.round(listing.sellerReputation ?? 0);
    ...
    card.innerHTML = `
        ...
        <div class="p-5 flex flex-col gap-3 flex-1">
            ...
            <h3 class="font-display text-base font-bold text-uce-navy line-clamp-2">
                ${listing.title}
            </h3>
            ...
            <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                ...
                <span class="font-bold text-emerald-600 text-sm ${isSold ? 'opacity-50' : ''}">
        ...
        <div class="px-5 pb-5">
            <button
                ...
                ${isSold ? 'bg-gray-100 text-gray-400' : 'bg-uce-navy text-uce-gold hover:bg-uce-navy-light'}">
...
async function loadCatalog() {
    ...
    container.innerHTML = Array(6)
        .fill(0)
        .map(() => `<div class="skeleton rounded-2xl h-72"></div>`)
        .join('');
...
function renderCatalog(listings) {
    ...
    for (let i = 0; i < listings.length; i++) {
        fragment.appendChild(createCard(listings[i]));
    }
